...
public class PersonTypeDaoImpl implements PersonTypeDao 
{
	...
	public PersonTypeDaoImpl(final Connection connection) 
	{
		super();
	...
	public List<Combobox> getAllCombobox(final String objectType) throws Exception 
	{
		...
		query.append("{");
		query.append("call pr_person_type");
		query.append("(");
		...
		query.append("? "); //05 - IN _object_type VARCHAR(3)
		query.append(")");
		query.append("}");
		...
		{			
			...
			statement.setString(5,  objectType);//05 - IN _object_type VARCHAR(3)
	        ...
	        while (resultSet.next()) 
	        {
	        	items.add(new Combobox(resultSet.getString(DatabaseConstants.COLUMN_ID), resultSet.getString(DatabaseConstants.COLUMN_DESCRIPTION),
	        			 resultSet.getString(DatabaseConstants.COLUMN_OBJECT_TYPE)));
	        }	        
		}
	...
	public Integer save(final PersonTypeDTO personType) throws Exception 
	{
		...
		try
		{
			...
			statement.setString(2, personType.getDescription()); //02 - IN _description varchar(100), 
			statement.setInt(3, personType.getInactiveInt()); //03 - IN _inactive int, 
			statement.setLong(4, personType.getChangeData().getUserChange()); //04 - IN _user_change BIGINT,
			statement.setString(5, personType.getObjectType()); //02 - IN _object_type VARCHAR(3)
			final boolean isProcessed  = statement.execute();
	        
	    	if(isProcessed) {
	    		resultSet = statement.getResultSet();
				while (resultSet.next()) {
					personTypeId = resultSet.getInt(DatabaseConstants.COLUMN_PERSON_TYPE_ID);
				}
			}
		}
		catch(final Exception ex)
