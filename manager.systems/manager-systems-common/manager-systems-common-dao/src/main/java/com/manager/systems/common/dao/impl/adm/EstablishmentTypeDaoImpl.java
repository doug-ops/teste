...
public class EstablishmentTypeDaoImpl implements EstablishmentTypeDao 
{
	private Connection connection;
	
	public EstablishmentTypeDaoImpl(final Connection connection) 
	{
		super();
	...
	public List<Combobox> getAllCombobox() throws Exception 
	{
		...
		query.append("{");
		query.append("call pr_establishment_type");
		query.append("(");
	...
	public Integer save(final EstablishmentTypeDTO establishmentType) throws Exception 
	{
		...
		try
		{
			...
			statement.setString(2, establishmentType.getDescription()); //02 - IN _description varchar(100), 
			statement.setInt(3, establishmentType.getInactiveInt()); //03 - IN _inactive int, 
			statement.setLong(4, establishmentType.getChangeData().getUserChange()); //04 - IN _user_change BIGINT,
