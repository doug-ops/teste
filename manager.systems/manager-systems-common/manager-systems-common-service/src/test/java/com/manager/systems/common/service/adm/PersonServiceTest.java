...
public class PersonServiceTest 
{
	...
	public void inicializa() throws Exception
	{
		...
		this.personService = new PersonServiceImpl(this.connection);
	...
	{		
		...
		final boolean isSaved = this.personService.save(person);
		...
		person.setUserChange(1L);
		final boolean isInactive = this.personService.inactive(person);
	    ...
	    person.setLastAccessDate(Calendar.getInstance());
	    person.setNegativeClose(false);		
		final boolean isUpdated = this.personService.save(person);
		...
		this.personService.getAll(reportPerson);
		Assert.assertTrue(reportPerson.getPersons().size()>0);	
		
		final List<Combobox> itens = this.personService.getAllAutocomplete(reportPerson);
