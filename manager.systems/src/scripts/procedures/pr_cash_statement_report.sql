...
DELIMITER $$
USE `manager`$$
CREATE PROCEDURE `pr_cash_statement_report` (IN _operation TINYINT, 
...
BEGIN    
	...
	IF(1 = _operation) THEN /** REPORT SINTETIC */          
        ...
        /**VARIABLES CURSOR*/
              ...
              INSERT INTO tmp_movement_analitic(company_id, movement_type_id, total, week_year, date_from, date_to, group_type_id, payment_data, provider_id, financial_group_id, financial_sub_group_id, bank_account_id, bank_account_origin_id, document_parent_id, document_type, document_note, transaction_same_company)
              SELECT 
				f.company_id, 
                f.movement_type_id, 
                ...
                f.date_to, 
                f.group_type_id,
                m.cashier_closing_date payment_data, 
                m.provider_id, 
                m.financial_group_id, 
                m.financial_sub_group_id, 
                m.bank_account_id, 
                m.bank_account_origin_id, 
                m.document_parent_id, 
                m.document_type,  
                m.document_note,
                0 transaction_same_company
              FROM 
              (
				  SELECT 
					company_id, week_year, date_from, date_to, movement_type_id, group_type_id FROM tmp_movement_sintetic s 
				  WHERE 
					s.company_id <> _company_id AND s.movement_type_id = 1 AND group_type_id = 1
              ...
              WHERE
					m.creation_date BETWEEN DATE_ADD(CAST(CONCAT(DATE_FORMAT(f.date_from,'%Y-%m-%d'), ' 00:00:00') AS DATETIME), INTERVAL -7 DAY) 
								   AND DATE_ADD(CAST(CONCAT(DATE_FORMAT(f.date_to,'%Y-%m-%d'), ' 23:59:59') AS DATETIME), INTERVAL -7 DAY) 
					AND m.payment_residue = 1 AND is_cashing_close = 1 AND m.inactive = 0;
			 ...
			 /* End Insert Saldo Anterior Fiados */
			  ...
			  /* Recebimento Previsto = Valores das duplicatas geradas em execução das lojas. Essa que na planilha onde tem os valores de pendências está descrimada como “duplicatas”  */
               ...
               SELECT 
                ...
                x.week_year, x.date_from, x.date_to, x.group_type_id,
                ...
                ms.document_parent_id, MAX(ms.document_type) document_type,  'FATURAMENTO LOJA' document_note, 0 transaction_same_company
                FROM 
                (              
				  SELECT 
					cf.company_id, m.document_parent_id, cf.week_year, cf.date_from, cf.date_to, cf.movement_type_id, cf.group_type_id
				  FROM 
					(
						  SELECT 
							company_id, week_year, date_from, date_to, movement_type_id, group_type_id FROM tmp_movement_sintetic s 
						  WHERE 
							s.company_id <> _company_id AND s.movement_type_id = 2 AND group_type_id = 1
						  GROUP BY company_id, week_year, date_from, date_to, movement_type_id, group_type_id
					) cf
              ...
              GROUP BY ms.document_parent_id, x.company_id, x.week_year, x.date_from, x.date_to, x.movement_type_id, x.group_type_id;
			 /* End Insert Recebimento Previsto */
             
             /* Insert despesas loja */
             INSERT INTO tmp_movement_analitic(company_id, movement_type_id, total, week_year, date_from, date_to, group_type_id, payment_data, provider_id, financial_group_id, financial_sub_group_id, bank_account_id, bank_account_origin_id, document_parent_id, document_type, document_note, transaction_same_company)
             SELECT 
				f.company_id, f.movement_type_id, 
                CASE WHEN credit = 1 THEN m.document_value ELSE (m.document_value * -1) END total,
                f.week_year, f.date_from, f.date_to,  f.group_type_id,				
                m.payment_data, m.provider_id, 
				m.financial_group_id, 
                m.financial_sub_group_id, 
                m.bank_account_id, 
                m.bank_account_origin_id, 
              ...
              WHERE
					...
					AND m.company_id <> _company_id
                    AND is_cashing_close = 1 AND document_type = 3 AND m.document_status = 2 
			...
			SELECT
                ...
                -- YEARWEEK(DATE_ADD(DATE(a.payment_data), INTERVAL (CASE WHEN WEEKDAY(DATE(a.payment_data)) = 6 THEN 6 ELSE 7 END) DAY)) week_year,
                a.week_year, date_format(a.date_from,'%d/%m/%y') date_from, date_format(a.date_to,'%d/%m/%y') date_to, a.group_type_id,
...
CALL pr_cash_statement_report(1, 182, '2024-01-01 00:00:00', '2024-01-07 23:59:59', '8');