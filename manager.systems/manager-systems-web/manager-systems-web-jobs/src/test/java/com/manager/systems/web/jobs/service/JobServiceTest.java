...
public class JobServiceTest 
{
	...
	{		
		...
		Assert.assertNotNull(this.jobService);
		...
		job.setUserChange(1);
		boolean isSaved = this.jobService.save(job, connection);
		...
		job.setInactive(false);
		job.setUserChange(1);		
		this.jobService.get(job, connection);
		...
		job.setUserChange(1);		
		final boolean isInactive = this.jobService.inactive(job, connection);
		...
		job.setInactive(false);
		job.setUserChange(1);
		final boolean isUpdated = this.jobService.save(job, connection);
		Assert.assertTrue(isUpdated);		
		
		final List<JobDTO> jobs = this.jobService.getAll(connection);
