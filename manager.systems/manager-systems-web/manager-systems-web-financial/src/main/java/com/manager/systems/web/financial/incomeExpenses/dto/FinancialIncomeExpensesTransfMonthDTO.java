...
@Setter
@AllArgsConstructor
public class FinancialIncomeExpensesTransfMonthDTO implements Serializable {
	
	private static final long serialVersionUID = -5735462720170731775L;
	...
	public void calculateTotais() {
		...
		if(this.companyId == 199) {
			this.balance = new BigDecimal(this.balance).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
			this.income = new BigDecimal((this.balance * -1)).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
			this.balance = (this.income + this.transfInput - this.expenses - this.transfOutput + this.profitDistribution);
			this.balance = new BigDecimal(this.balance).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
			
			this.items.add(0, FinancialIncomeExpensesItemDetailDTO.builder()
					.movementTypeId(1)
					.paymentDate(this.items != null && this.items.size() > 0 ? this.items.get(0).getPaymentDate() : ConstantDataManager.TRACO)
					.paymentDateLong(0)
					.documentValue(this.income)
					.documentDescription(ConstantDataManager.LABEL_GROSS_PROFIT).build());					
		}
	}
