...
public class CashierClosingDTO 
{
	...
	private Map<Long, CashierClosingItemDTO> itens = new TreeMap<Long, CashierClosingItemDTO>();
	...
	public final Map<Long, CashierClosingItemDTO> getItens() 
	{
		return this.itens;
	}
	...
	public final String getUserParents() 
	{
		return this.userParents;
	}

	public final void setUserParents(final String userParents) 
	{
	...
	public final void addItem(final CashierClosingItemDTO item) 
	{
		...
		CashierClosingItemDTO value = null;
		long key = item.getCompanyId();
	...
	public final CashierClosingWekendDTO convetToGroupWeek() throws Exception {
		
		final CashierClosingWekendDTO movementWekend = new CashierClosingWekendDTO(); 		
		movementWekend.prepareMapDate(this.dateFrom, this.dateTo);
		
		for(final Map.Entry<Long, CashierClosingItemDTO> entry : this.itens.entrySet()) {
			movementWekend.addItemCashierClosing(entry.getValue());
		}
		return movementWekend;
	}
	
	public final CashierClosingCompanyDTO convetGroupCompany() throws Exception {
		
		final CashierClosingCompanyDTO movementDocumentCompany = new CashierClosingCompanyDTO(); 
		...
		for(final Map.Entry<Long, CashierClosingItemDTO> entry : this.itens.entrySet()) {
			movementDocumentCompany.addItemCashierClosing(entry.getValue());
